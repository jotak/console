...
const stubTFunc = (text, opts) => `${text}/${opts.path}`;
...
describe('NetworkPolicy model conversion', () => {
  it('should convert deny-all resource', () => {
    const converted = networkPolicyToK8sResource(denyAll, stubTFunc);
  ...
  it('should convert same-namespace frontend pods rule to K8s resource', () => {
    const converted = networkPolicyToK8sResource(sameNamespace, stubTFunc);
  ...
  it('should convert other-namespaces egress rule to K8s resource', () => {
    const converted = networkPolicyToK8sResource(otherNamespace, stubTFunc);
  ...
  it('should convert ipblock rule to K8s resource', () => {
    const converted = networkPolicyToK8sResource(ipBlockRule, stubTFunc);
  ...
  it('should ignore selector with empty key', () => {
    ...
    const converted = networkPolicyToK8sResource(policy, stubTFunc);
  ...
  it('should trigger error when converting invalid yaml', () => {
    ...
    expect(converted).toEqual({
      error: 'public~{{path}} should be an Array./Spec.ingress',
      kind: 'invalid',
    });
  });
});
